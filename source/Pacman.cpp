...
void Pacman::update(float deltaTime) {
    ...
    // Update power state // energized state type shii
    if (powered) {
    ...
    // Update position based on collision box
...
void Pacman::render(RenderWindow& window) {
    ...
    if (mouthOpen) {
        ...
        float mouthSize = 60.0f; // Larger mouth angle (in degrees)
        
        // Use a CircleShape with a higher point count for smoother appearance
        CircleShape newPacman(shape.getRadius(), 30); // 30 points for smoother circle
        newPacman.setFillColor(Color::Yellow);
        newPacman.setPosition(position.x - shape.getRadius(), position.y - shape.getRadius());
        
        // Create a triangle for the mouth
        ConvexShape mouth;
        ...
        mouth.setPoint(1, Vector2f(
            shape.getRadius() + cos(rad1) * shape.getRadius() * 1.2f,
            shape.getRadius() + sin(rad1) * shape.getRadius() * 1.2f
        ));
        
        mouth.setPoint(2, Vector2f(
            shape.getRadius() + cos(rad2) * shape.getRadius() * 1.2f,
            shape.getRadius() + sin(rad2) * shape.getRadius() * 1.2f
        ));
        ...
        // Draw a complete circle when mouth is closed
        CircleShape betterPacman(shape.getRadius(), 30); // 30 points for smoother circle
